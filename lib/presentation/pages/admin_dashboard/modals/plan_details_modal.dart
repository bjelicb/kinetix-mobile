...
import 'unassign_plan_modal.dart';
  ...
  await showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      content: SingleChildScrollView(
        child: isLoading
            ? const Padding(padding: EdgeInsets.all(AppSpacing.lg), child: AnimatedCyberLoader(size: 40))
            : Column(
                ...
                children: [
                  if (planData['description'] != null) ...[
                    Text(
                      ...
                      style: Theme.of(
                        context,
                      ).textTheme.titleSmall?.copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.bold),
                    ),
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: PlanDetailItem(label: 'Trainer', value: planData['trainerName'] ?? 'Unknown'),
                      ),
                      const SizedBox(width: 8),
                      Expanded(
                        child: PlanDetailItem(label: 'Difficulty', value: planData['difficulty'] ?? 'N/A'),
                      ),
                    ],
                  ),
                  const SizedBox(height: 8),
                  PlanDetailItem(label: 'Weekly Cost', value: '€${(planData['weeklyCost'] ?? 0).toStringAsFixed(2)}'),
                  const SizedBox(height: 8),
                  PlanDetailItem(
                    ...
                    ...(planData['workouts'] as List).map<Widget>((workout) {
                      return Padding(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(color: AppColors.surface1, borderRadius: BorderRadius.circular(8)),
                          child: Row(
                            children: [
                ...
                builder: (dialogContext) => AlertDialog(
                  ...
                  content: Row(
                    ...
                    children: [
                      ...
                      Text('Duplicating plan...', style: Theme.of(dialogContext).textTheme.bodyMedium),
                    ],
                  ),
                ),
              );
              ...
              try {
                ...
                if (modalContext.mounted) {
                  ScaffoldMessenger.of(modalContext).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Error duplicating plan: ${e.toString().replaceAll('Exception: ', '').split('\n').first}',
                      ),
                      backgroundColor: AppColors.error,
                ...
                builder: (dialogContext) => AlertDialog(
                  ...
                  content: Row(
                    ...
                    children: [
                      const CircularProgressIndicator(),
                      const SizedBox(width: 16),
                      Text('Deleting plan...', style: Theme.of(dialogContext).textTheme.bodyMedium),
                    ],
                  ),
                ),
              );
              ...
              try {
                ...
                if (modalContext.mounted) {
                  ScaffoldMessenger.of(modalContext).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Error deleting plan: ${e.toString().replaceAll('Exception: ', '').split('\n').first}',
                      ),
                      backgroundColor: AppColors.error,
                      duration: const Duration(seconds: 4),
                    ),
