...
Future<void> showAssignPlanModal({
  ...
  developer.log(
    '[AssignPlanModal] Total assignedClientProfileIds: ${assignedClientProfileIds.length}',
  ...
  for (final client in allClients) {
    ...
    developer.log(
      '[AssignPlanModal] Checking client ${client.name} (id: ${client.id}, clientProfileId: $clientProfileId)',
      name: 'AssignPlanModal',
    );
    ...
    if (assignedClientProfileIds.contains(clientProfileId)) {
      ...
      developer.log(
        '[AssignPlanModal] Matched! Client ${client.name} is assigned (userId: ${client.id})',
        name: 'AssignPlanModal',
      );
    }
  }
      ...
      return Padding(
        ...
        child: StatefulBuilder(
          builder: (context, setModalState) {
            ...
            developer.log(
              '[AssignPlanModal] Builder called - selectedClients: ${selectedClients.length}, assignedUserIds: ${assignedUserIds.length}',
              name: 'AssignPlanModal',
            );
              ...
              child: SingleChildScrollView(
                child: Column(
                        ...
                        children: [
                          Text(
                            ...
                            style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                              color: selectedStartDate == null ? AppColors.textSecondary : AppColors.textPrimary,
                            ),
                          ),
                          Icon(
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text('Select Clients:', style: Theme.of(context).textTheme.titleMedium),
                      if (selectedClients.isNotEmpty)
                        Container(
                          ...
                          child: Text(
                            ...
                            style: Theme.of(
                              context,
                            ).textTheme.bodySmall?.copyWith(color: AppColors.primary, fontWeight: FontWeight.bold),
                          ),
                        ),
                    ],
                  ),
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              if (index == 0 && isAlreadyAssigned) {
                                developer.log(
                                  '[AssignPlanModal] First assigned client - ${client.name}, isSelected: $isSelected, isAlreadyAssigned: $isAlreadyAssigned',
                                  name: 'AssignPlanModal',
                                );
                              }
                        ...
                        : () async {
                            try {
                              ...
                              final clientIds = selectedClients.map((clientId) {
                                ...
                                developer.log(
                                  '[AssignPlanModal] Client ${client.name} -> sending userId: ${client.id}',
                                  name: 'AssignPlanModal',
                                );
                                return client.id;
                              }).toList();
                              ...
                              await ref
                                  ...
                                  .assignPlanToClients(planId, clientIds, selectedStartDate!);
                              if (!context.mounted) return;
                              Navigator.pop(context);

                              developer.log(
                                '[AssignPlanModal] ✅ Plan assigned successfully to ${clientIds.length} client(s)',
                                name: 'AssignPlanModal',
                              );

                              // Sync workouts to get newly created workout logs
                              developer.log(
                                '[AssignPlanModal] 🔄 Syncing workouts after plan assignment...',
                                name: 'AssignPlanModal',
                              );
                              try {
                                ...
                                developer.log(
                                  '[AssignPlanModal] ✅ Workouts synced successfully',
                                  name: 'AssignPlanModal',
                                );
                              } catch (e) {
                                developer.log(
                                  '[AssignPlanModal] ⚠️ Error syncing workouts: $e',
                                  name: 'AssignPlanModal',
                                  error: e,
                                );
                              ...
                              // Add delay to ensure backend has time to create workout logs
                              developer.log(
                                '[AssignPlanModal] ⏳ Waiting 1500ms before refreshing workout list (backend needs time to create 7 workout logs)...',
                                name: 'AssignPlanModal',
                              );
                              await Future.delayed(const Duration(milliseconds: 1500));
                              developer.log(
                                '[AssignPlanModal] ⏳ Delay completed, proceeding with refresh...',
                                name: 'AssignPlanModal',
                              );
                              ...
                              if (onRefreshWorkouts != null) {
                                developer.log(
                                  '[AssignPlanModal] 🔄 Calling onRefreshWorkouts() callback to refresh workout list...',
                                  name: 'AssignPlanModal',
                                );
                                try {
                                  await onRefreshWorkouts();
                                  developer.log(
                                    '[AssignPlanModal] ✅ onRefreshWorkouts() completed successfully',
                                    name: 'AssignPlanModal',
                                  );
                                } catch (e) {
                                  developer.log(
                                    '[AssignPlanModal] ❌ Error in onRefreshWorkouts(): $e',
                                    name: 'AssignPlanModal',
                                    error: e,
                                  );
                                }
                                ...
                                developer.log(
                                  '[AssignPlanModal] ⚠️ onRefreshWorkouts callback is null, skipping direct refresh',
                                  name: 'AssignPlanModal',
                                );
                              }

                              // Then call general onAssigned callback
                              developer.log(
                                '[AssignPlanModal] 🔄 Calling onAssigned() callback to refresh other lists...',
                                name: 'AssignPlanModal',
                              );
                              try {
                                await onAssigned();
                                developer.log(
                                  '[AssignPlanModal] ✅ onAssigned() callback completed',
                                  name: 'AssignPlanModal',
                                );
                              } catch (e) {
                                developer.log(
                                  '[AssignPlanModal] ❌ Error in onAssigned(): $e',
                                  name: 'AssignPlanModal',
                                  error: e,
                                );
                              }
                              ...
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Plan assigned successfully. Workout logs are being synced...'),
                                    ...
                                    SnackBar(
                                      content: Column(
                                        ...
                                        children: [
                                          ...
                                          const SizedBox(height: 4),
                                          Text(errorMessage, style: TextStyle(fontSize: 14)),
                                        ],
                                      ),
